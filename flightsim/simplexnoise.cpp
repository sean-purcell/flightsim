...
bool Simplex::initialized = false;
...
Simplex::Simplex(double _freq, double _persist,
	double _amp, int _octaves, int _seed)
	: freq(_freq), amp(_amp), persistence(_persist),
	octaves(_octaves), seed(_seed) {
...
double Simplex::getValue(double x, double y) {
	...
	for(int i = 0; i < octaves; i++) {
		res += noise(_freq * x + seed, _freq * y + seed) * _amp;
		_amp *= persistence;
		_freq *= 2;
	}
